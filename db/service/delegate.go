...
func NewDelegate() *Delegate {
	return &Delegate{
		tableName: (&model.Delegate{}).TableName(),
	}
}

// List 列表
func (object *Delegate) List(db *gorm.DB, offset, limit int) (total int, list []*model.Delegate, err error) {
	var count int
...
// Add 添加
func (object *Delegate) Add(db *gorm.DB, delegate *model.Delegate) (err error) {
	err = db.Create(delegate).Error
	return
}

// AddAll 添加所有
func (object *Delegate) AddAll(db *gorm.DB, list []*model.Delegate) (err error) {
