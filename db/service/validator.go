...
func NewValidator() *Validator {
	return &Validator{
		tableName: (&model.Validator{}).TableName(),
	}
}

// List 列表
func (object *Validator) List(db *gorm.DB, status, offset, limit int) (total int, list []*model.Validator, err error) {
	var count int
...
// Add 添加
func (object *Validator) Add(db *gorm.DB, validator *model.Validator) (err error) {
	err = db.Create(validator).Error
	return
}

// AddAll 添加所有
func (object *Validator) AddAll(db *gorm.DB, list []*model.Validator) (err error) {
	err = db.Create(list).Error
	return
}

// UpdateStatus 更新状态
func (object *Validator) UpdateStatus(db *gorm.DB, validator *model.Validator) (err error) {
	err = db.Model(&model.Validator{}).
