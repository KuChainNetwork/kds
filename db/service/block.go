...
func NewBlock() *Block {
	return &Block{
		tableName: (&model.Block{}).TableName(),
	}
}
...
// Latest
func (object *Block) Latest(db *gorm.DB) (m *model.Block, err error) {
	var list []*model.Block
	if err = db.Model(&model.Block{}).
		Order("Height DESC").
		Limit(1).
...
// List
func (object *Block) List(db *gorm.DB, offset, limit, sortByHeight int) (total int, list []*model.Block, err error) {
	var count int
...
// AddAll
func (object *Block) AddAll(db *gorm.DB, blocks []*model.Block) (err error) {
